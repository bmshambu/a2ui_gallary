...
def image_messages() -> list[dict]:
    ...
    components = [
        ...
        _text("header", "Image component", usage_hint="h4"),
        {
            ...
            "component": {
                "Image": {
                    # Base64 PNG data URI (from agent/demo_image.py, generated by
                    # encode_image.py). No external fetch — GE blocks external
                    # image hosts and 500s on failed fetches. Swap the image by
                    # running: python encode_image.py path/to/your-image.png
                    "url": {"literalString": DEMO_IMAGE_DATA_URI},
                    "fit": "contain",
                }
            },
        },
        _text(
            "caption",
            "This image is embedded inline as a base64 data URI (no external URL).",
            usage_hint="caption",
        ),
    ]
    return [
